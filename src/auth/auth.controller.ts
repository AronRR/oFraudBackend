...
export class AuthController{
    ...
    async login(@Body() dto:{email:string, password:string}){
        ...
        const userProfile = {id: usuario.id.toString(), email: usuario.email, name: usuario.name, role: usuario.role ?? "user"};
    ...
    async refresh(@Body() dto: {refreshToken: string}){
        try{
            ...
            const newAccessToken = await this.tokenService.generateAccess({id: user.id.toString(), email: user.email, name: user.name, role: user.role ?? "user"});
