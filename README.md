...
El panel de superadministraci�n expone un listado con cada acci�n relevante registrada por el backend.
...
Acciones que generan registros autom�ticos:

| Acci�n | Tipo guardado | Descripci�n |
| --- | --- | --- |
| Login de admin/superadmin | `login` | Cada autenticaci�n exitosa |
| Moderaci�n de reportes | `approve_report` / `reject_report` | Registra estado previo, revisi�n y notas |
| Eliminaci�n de reportes | `delete_report` | Guarda estado previo antes de eliminar |
| Bloqueo / desbloqueo de usuarios | `block_user` / `unblock_user` | Incluye motivo cuando se proporciona |
| Creaci�n / actualizaci�n de categor�as | `create_category` / `update_category` | Detalla slug y cambios aplicados |
| Resoluci�n de flags | `resolve_flag` | Indica estado anterior y nuevo del flag |
| Promoci�n / degradaci�n de administradores | `promote_user` / `demote_user` | Guarda roles de origen/destino y motivos |
